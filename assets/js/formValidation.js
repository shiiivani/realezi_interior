...
$(document).ready(function () {
  $('.gradient-content-section #consultation').on('click', function (e) {
    e.preventDefault();
    $('.share-detail-form').addClass("show");
  });
  $('#expertTalk').on('click', function (e) {
    e.preventDefault();
    $('.expert-talk-form').addClass("show");
  });
  $('#freeConsultation').on('click', function (e) {
    e.preventDefault();
    $('.free-consultation-form').addClass("show");
  });
  $('.close-icon i').on('click', function () {
    $('.share-detail-form').removeClass("show");
    $('.expert-talk-form').removeClass("show");
    $('.free-consultation-form').removeClass("show");
  });
});
$(document).ready(function () {
  $('#shareDetailBtn').on('click', function (e) {
    e.preventDefault();
    // $("#shareDetailForm").fadeOut()
    $('.great-msg').fadeIn().css('display', 'flex');
  });
  $('#ExpertDetailBtn').on('click', function (e) {
    e.preventDefault();
    $('.expert-great-msg').fadeIn().css('display', 'flex');
  });
  $('#freeConsultationBtn').on('click', function (e) {
    e.preventDefault();
    $('.consultation-great-msg').fadeIn().css('display', 'flex');
  });
});
...
const propertybox = document.querySelectorAll(".property-type-dropdown .propertyType-list")
const propertyList = document.querySelectorAll(".property-type-dropdown .propertyType-list ul li");
    ...
    list.addEventListener("click", (event) => {
      ...
      if (item) {
        if (formId === 'shareDetailForm') {
          ...
          document.getElementById("property").textContent = event.target.textContent
        }
     
        if(formId === "expertForm") {

          document.getElementById("property-expert").textContent = event.target.textContent
        }
...
document.addEventListener('DOMContentLoaded', () => {
  const form = document.getElementById('shareDetailForm');
  const submitBtn = document.getElementById('shareDetailBtn');
  const requiredFields = Array.from(form.querySelectorAll('input[required], select[required]'));

  // Function to check form validity
  const checkFormValidity = () => {
    // Check if all required fields are filled
    const isValid = requiredFields.every(field => field.value.trim() !== '' && (field.type !== 'checkbox' || field.checked));
...
// free-consulatation js start
document.addEventListener('DOMContentLoaded', () => {
  const form = document.getElementById('freeConsultationForm');
  const submitButton = document.getElementById('freeConsultationBtn');
  const requiredFields = Array.from(form.querySelectorAll('input[required], select[required]'));
